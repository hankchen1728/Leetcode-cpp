...
- Link: [https://leetcode.com/problems/kth-smallest-number-in-multiplication-table/](https://leetcode.com/problems/kth-smallest-number-in-multiplication-table/)

## Idea
For each time, calculate the number of elements in the table are lower than or equal to the `mid` (`( left + right ) / 2`).

If the count is lower than `k`, i.e. the `mid` is lower than our target (the $k$-th smallest element), then update the search region boundary `left` to be `mid`.
