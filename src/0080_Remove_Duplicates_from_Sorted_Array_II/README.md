...
## Idea
Two pointers, one for the position of element to be replaced `L`, the other for the position to move (faster) `R`.

Before doing the replacement `nums[L] = nums[R]`, we need to check whether `nums[R] > nums[L - 2]` or not. If `nums[R] == nums[L - 2]`, it will have three same element togethor, which is contract to the requirement of the problem description.
