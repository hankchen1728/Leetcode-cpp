...
## Idea
First find the monotonic decreasing part from right most side. And then reverse the subarray.

For the following example, reverse the part `[5, 4, 3, 2, 1]` to be `[1, 2, 3, 4, 5]`.
```
[1, 2, 5, 4, 3, 2, 1]   =>   [1, 2, 1, 2, 3, 4, 5]   =>   [1, 3, 1, 2, 2, 4, 5]
       ^           ^             ^        ^
```
Now we need to choose a smallest number larger than `2` from the subarray, and then swap it with `2`.

Finally we will get the result `[1, 3, 1, 2, 2, 4, 5]`.
